...
function AddAssetTagsForTemplates{
    ...
    process{
        ...
        if( ($vsixXml.PackageManifest.Assets.Asset | Where-Object {$_.Path -eq 'PecanWaffle.dll'}) -eq $null ){
            # create the element here
            $projElement = $vsixXml.CreateElement('Asset', $vsixXml.DocumentElement.NamespaceURI)
            $projElement.SetAttribute('Type', 'Microsoft.VisualStudio.Assembly') | Out-Null
            $projElement.SetAttribute('Source', 'File') | Out-Null
            $projElement.SetAttribute('Path', 'PecanWaffle.dll') | Out-Null
            $projElement.SetAttribute('AssemblyName', 'PecanWaffle, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null') | Out-Null
            ...
            if($assetTag){
                ...
                'Unable to add PecanWaffle.dl sset tag to the .vsixmanifest file, please add them manually' | Write-Warning
            }
        }
